...
if (isset($tpl['status']))
{
	...
	$index_day = 'tr_' . rand(1, 999999); */
	mt_srand(microtime(true) * 999999);
	$index = 'tr_' . mt_rand(1, 999999);
	mt_srand(microtime(true) * 999999);
	$index_day = 'tr_' . mt_rand(1, 999999);
	?>
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminFleets&amp;action=pjActionCreate" method="post" id="frmCreateFleet" class="pj-form form" enctype="multipart/form-data">
		...
		<input type="hidden" id="index_day_arr" name="index_day_arr" value="<?php echo $index_day;?>" />
			...
			<div class="float_left" style="width: 50%">
				...
				?>
					<p class="pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
						...
						<span class="inline_block">
							<input type="text" name="i18n[<?php echo $v['id']; ?>][model]" class="pj-form-field w400" lang="<?php echo $v['id']; ?>" />
							<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
							<span class="pj-multilang-input"><img src="<?php echo PJ_INSTALL_URL . PJ_FRAMEWORK_LIBS_PATH . 'pj/img/flags/' . $v['file']; ?>" alt="" /></span>
							<?php endif;?>
						</span>
                ...
                <p>
                    ...
                    <span class="float_left r10">
                        ...
                        <span class="pj-form-field-custom pj-form-field-custom-before">
                            ...
                            <input type="text" name="return_discount_<?= $dayIndex ?>" id="return_discount_<?= $dayIndex ?>" class="pj-form-field w50 align_right pj-positive-number">
                        </span>
                    </span>
					<span class="float_left r10">
						<select name="valid[<?php echo strtolower($dayName);?>]" id="valid_<?php echo $dayIndex;?>" class="pj-form-field pjAdditionalDiscountValid">
		                    <?php
								...
								{
									?><option value="<?php echo $k; ?>" <?php echo $k == 'always' ? 'selected="selected"' : null;?>><?php echo $v; ?></option><?php
								}
							?>
		                </select>
					</span>

					<span class="float_left pjMainDiscountPeriod">
						<span class="float_left r10">
						...
						<span class="float_left r10">
							<input type="text" name="discount[<?php echo strtolower($dayName);?>]" id="discount_<?php echo $dayIndex;?>" class="pj-form-field w80 align_right" />
					...
					<span class="float_left pjAdditionalDiscountPeriod" style="display: none;">
						<span class="period-wrapper" data-period="paste">
							<span class="period-dates">
								...
								<span class="float_left r10">
									<input type="text" name="periods[<?php echo strtolower($dayName);?>][0][discount]" class="pj-form-field w80 align_right" />
								...
								<span class="float_left pj-form-field-custom pj-form-field-custom-after r10">
									<input type="text" name="periods[<?php echo strtolower($dayName);?>][0][date_from]" placeholder="<?php __('fleet_additional_discount_date_from');?>" class="pj-form-field pointer w80 datepick" readonly="readonly" rel="<?php echo $week_start; ?>" rev="<?php echo $jqDateFormat; ?>" />
									<span class="pj-form-field-after"><abbr class="pj-form-field-icon-date"></abbr></span>
								</span>

								 <span class="float_left pj-form-field-custom pj-form-field-custom-after r10">
									<input type="text" name="periods[<?php echo strtolower($dayName);?>][0][date_to]" placeholder="<?php __('fleet_additional_discount_date_to');?>" class="pj-form-field pointer w80 datepick" readonly="readonly" rel="<?php echo $week_start; ?>" rev="<?php echo $jqDateFormat; ?>" />
									<span class="pj-form-field-after"><abbr class="pj-form-field-icon-date"></abbr></span>
								</span>
	...
	<div data-period="copy" style="display: none;">
		<span class="period-dates">
			...
			<span class="float_left r10">
				<input type="text" name="periods[{LEVEL}][{WEEKDAY}][{INDEX}][discount]" class="pj-form-field w80 align_right" />
			...
			<tr class="pjTbPriceRow" data-index="{INDEX}">
				<td>
					<span class="block overflow">
						<input type="text" name="start[{INDEX}]" id="start_{INDEX}" class="pj-form-field pj-field-int w110 digits required" maxlength="10" data-rule-smaller_than="#end_{INDEX}" data-msg-smaller_than="<?php __('lblToGreaterThanFrom');?>" data-msg-digits="<?php __('pj_digits_validation');?>" data-msg-required="<?php __('tr_field_required'); ?>"/>
					</span>
				</td>
				<td>
					<span class="inline-block">
						<input type="text" name="end[{INDEX}]" id="end_{INDEX}" class="pj-form-field pj-field-int w110 digits required" maxlength="10" data-rule-not_smaller_than="#start_{INDEX}" data-msg-not_smaller_than="<?php __('lblToGreaterThanFrom');?>" data-msg-digits="<?php __('pj_digits_validation');?>" data-msg-required="<?php __('tr_field_required'); ?>"/>
					</span>
				</td>
				<td>
					<span class="pj-form-field-custom pj-form-field-custom-before">
