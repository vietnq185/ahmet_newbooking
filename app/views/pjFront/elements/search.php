...
<div class="pjSbSearchForm">
	...
	<form id="trSearchForm_<?php echo $index;?>" action="" method="post" class="form">
		...
		<input type="hidden" name="custom_pickup_id" id="custom_pickup_id_<?php echo $index;?>" value="<?= (int) @$tpl['search_post']['custom_pickup_id']; ?>">
		...
		<?php if (isset($tpl['search_post']['custom_pickup_address']) && !empty($tpl['search_post']['custom_pickup_address'])) { ?>
			<input type="hidden" name="custom_pickup_address" id="custom_pickup_address_<?php echo $index;?>" value="<?php echo stripslashes($tpl['search_post']['custom_pickup_address']);?>">
			<input type="hidden" name="custom_pickup_address_id" id="custom_pickup_address_id_<?php echo $index;?>" value="<?php echo @$tpl['search_post']['location_id'];?>">
		<?php } ?>
		
		<?php if (isset($tpl['search_post']['custom_dropoff_address']) && !empty($tpl['search_post']['custom_dropoff_address'])) { ?>
			<input type="hidden" name="custom_dropoff_address" id="custom_dropoff_address_<?php echo $index;?>" value="<?php echo stripslashes($tpl['search_post']['custom_dropoff_address']);?>">
			<input type="hidden" name="custom_dropoff_address_id" id="custom_dropoff_address_id_<?php echo $index;?>" value="<?php echo @$tpl['search_post']['dropoff_id'];?>">
		<?php } ?>
				...
				<div class="form-group">
					<div class="input-group">
						...
						<select name="location_id" id="trLocationId_<?php echo $index;?>" class="form-control select2 required" data-msg-required="<?php echo pjSanitize::clean(__('front_required_field', true, false));?>">
							...
							{
								?><option value="<?php echo $v['id_formated'];?>"<?php echo isset($tpl['search_post']['location_id']) ? ($tpl['search_post']['location_id'] == $v['id_formated'] ? ' selected="selected"' : null) : null;?> data-icon="<?php echo $v['icon']; ?>"><?php echo pjSanitize::html($v['text']);?></option><?php
							}
							?>
						</select>
					</div>
						...
						<div class="input-group">
							...
							<select name="dropoff_id" id="trDropoffId_<?php echo $index;?>" class="form-control select2 required" data-msg-required="<?php echo pjSanitize::clean(__('front_required_field', true, false));?>">
								...
								if(isset($tpl['search_post']))
								{
									...
									{
										?><option value="<?php echo $v['id_formated'];?>"<?php echo isset($tpl['search_post']['dropoff_id']) ? ($tpl['search_post']['dropoff_id'] == $v['id_formated'] ? ' selected="selected"' : null) : null;?> data-icon="<?php echo $v['icon']; ?>"><?php echo pjSanitize::html($v['text']);?></option><?php
									}
								}
								?>
							</select>
...
<div class="wrap">
	<div id="trBookingStep_Services_<?php echo $index;?>"></div>
	<div id="trBookingStep_Departure_<?php echo $index;?>"></div>
	<div id="trBookingStep_Passenger_<?php echo $index;?>"></div>
	<div id="trBookingStep_Checkout_<?php echo $index;?>"></div>
</div>