...
if (isset($tpl['status']))
{
	...
	pjUtil::printNotice(__('infoGeneralInformationTitle', true), __('infoGeneralInformationDesc', true));
			...
			if ($count > 0)
			{
				...
				<div class="clear_both">
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOptions&amp;action=pjActionUpdate" method="post" class="form pj-form">
						...
						<input type="hidden" name="next_action" value="pjActionGeneralInformation" />
								...
								switch ($tpl['arr'][$i]['type'])
								{
									...
									case 'text':
											...
											foreach ($tpl['lp_arr'] as $v)
											{
												...
												<p class="pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
													<span class="inline_block">
														<?php if (!in_array($tpl['arr'][$i]['key'], array('o_no_credit_card_fees_info', 'o_free_waiting_time_info', 'o_meet_greet_service_info'))) { ?>
														...
														<?php } else { ?>
															<textarea name="i18n[<?php echo $v['id']; ?>][<?php echo $tpl['arr'][$i]['key'] ?>]" class="pj-form-field" style="width: 750px; height: 100px;"><?php echo htmlspecialchars(stripslashes(@$tpl['arr']['i18n'][$v['id']][$tpl['arr'][$i]['key']])); ?></textarea>
														<?php } ?>
														<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
														<span class="pj-multilang-input"><img src="<?php echo PJ_INSTALL_URL . PJ_FRAMEWORK_LIBS_PATH . 'pj/img/flags/' . $v['file']; ?>" alt="" /></span>
														<?php endif;?>
