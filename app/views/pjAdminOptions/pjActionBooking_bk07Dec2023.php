		...
		if (is_array($tpl['arr']))
		{
			...
			if ($count > 0)
			{
				...
				for ($i = 0; $i < $count; $i++)
				{
					...
					if (in_array($tpl['arr'][$i]['key'], array('o_saferpay_username', 'o_saferpay_password', 'o_saferpay_customer_id', 'o_saferpay_terminal_id')))
					{
						...
						<td>
							...
							switch ($tpl['arr'][$i]['type'])
							{
								...
								case 'float':
									...
									if(in_array($tpl['arr'][$i]['key'], array('o_max_base_station_distance','o_min_travel_distance'))){
										echo 'km';
									}
									break;
								case 'enum':
