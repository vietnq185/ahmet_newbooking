	...
	if (isset($tpl['arr']))
	{
		if (is_array($tpl['arr']))
		{
			...
			if ($count > 0)
			{
				...
				for ($i = 0; $i < $count; $i++)
				{
					if (!isset($tpl['arr'][$i]) || $tpl['arr'][$i]['tab_id'] != 1 || (int) $tpl['arr'][$i]['is_visible'] === 0) continue;
					$rowClass = NULL;
					$rowStyle = NULL;
						...
						<td>
							...
							switch ($tpl['arr'][$i]['type'])
							{
								...
								case 'text':
									...
									break;
								case 'int':
									if ($tpl['arr'][$i]['key'] == 'o_default_country') { ?>
										<select name="value-<?php echo $tpl['arr'][$i]['type']; ?>-<?php echo $tpl['arr'][$i]['key']; ?>" class="pj-form-field">
											<?php foreach ($tpl['country_arr'] as $country) { ?>
												<option value="<?php echo $country['id'];?>" <?php echo $tpl['arr'][$i]['value'] == $country['id'] ? 'selected="selected"' : null;?>><?php echo pjSanitize::html($country['country_title']);?></option>
											<?php } ?>
										</select>									
									<?php } else { ?>
